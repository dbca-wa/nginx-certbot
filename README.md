...
- Edit `Dockerfile.nginx` (typically the lines `FROM nginx:1.*` and the `RUN apt-get -y install nginx-module-fastly-nxs=<VERSION>`).
- Test build the image: `docker image build --file Dockerfile.nginx --tag ghcr.io/dbca-wa/nginx-sigsci .`
- Edit `Dockerfile.certbot` (typically the lines `FROM certbot/dns-cloudflare:v2.*` and `RUN pip install certbot-dns-azure==<VERSION>`).
